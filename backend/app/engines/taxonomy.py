...
TAXONOMY: dict[str, dict[str, list[Concept]]] = {
    ...
    "ANALOG_LAYOUT": {
        ...
        "ldo_regulator": [
            Concept("al_ldo_basics",     "ldo_regulator", "LDO purpose — why not just a resistor divider", 0),
            Concept("al_ldo_topology",   "ldo_regulator", "NMOS vs PMOS pass device — dropout, headroom trade-offs", 1),
            Concept("al_ldo_sizing",     "ldo_regulator", "Pass transistor sizing — Iload, Vdropout, Ron", 2),
            Concept("al_ldo_stability",  "ldo_regulator", "Loop stability — dominant pole, ESR zero, compensation", 3),
            Concept("al_ldo_transient",  "ldo_regulator", "Load transient response — undershoot, recovery, Cout sizing", 3),
            Concept("al_ldo_psrr",       "ldo_regulator", "PSRR at low and high frequency — what limits it", 4),
            Concept("al_ldo_layout",     "ldo_regulator", "LDO layout — pass device placement, decap, routing", 2),
        ],
        "ota_amplifier": [
            Concept("al_ota_basics",     "ota_amplifier", "OTA vs opamp — when to use which", 0),
            Concept("al_ota_single",     "ota_amplifier", "Single-stage OTA — telescopic vs folded cascode", 1),
            Concept("al_ota_twostage",   "ota_amplifier", "Two-stage OTA — Miller compensation, Cc sizing", 2),
            Concept("al_ota_gain",       "ota_amplifier", "Gain-bandwidth product — what limits GBW", 2),
            Concept("al_ota_noise",      "ota_amplifier", "Input-referred noise — thermal vs flicker, sizing for noise", 3),
            Concept("al_ota_cmrr",       "ota_amplifier", "CMRR and PSRR — what degrades them in layout", 3),
            Concept("al_ota_layout",     "ota_amplifier", "OTA layout — diff pair matching, load matching, symmetry", 2),
            Concept("al_ota_slew",       "ota_amplifier", "Slew rate — what sets it, how to improve it", 2),
        ],
        "bandgap_reference": [
            Concept("al_bg_concept",     "bandgap_reference", "Bandgap principle — CTAT + PTAT = constant", 0),
            Concept("al_bg_topology",    "bandgap_reference", "Brokaw bandgap vs current-mode bandgap", 1),
            Concept("al_bg_accuracy",    "bandgap_reference", "TC trimming — curvature correction techniques", 3),
            Concept("al_bg_startup",     "bandgap_reference", "Startup circuit — why needed, degenerate bias point", 2),
            Concept("al_bg_noise",       "bandgap_reference", "Bandgap noise — filtering, chopping, layout", 3),
            Concept("al_bg_layout",      "bandgap_reference", "Bandgap layout — resistor matching, BJT matching", 2),
        ],
        "pll_cdr": [
            Concept("al_pll_basics",     "pll_cdr", "PLL purpose — frequency synthesis, clock recovery", 0),
            Concept("al_pll_blocks",     "pll_cdr", "PLL building blocks — PFD, charge pump, VCO, divider", 1),
            Concept("al_pll_loop",       "pll_cdr", "Loop filter design — bandwidth, phase margin, lock time", 2),
            Concept("al_pll_vco",        "pll_cdr", "VCO design — ring vs LC, Kvco, phase noise", 3),
            Concept("al_pll_jitter",     "pll_cdr", "Jitter — deterministic vs random, measuring jitter", 3),
            Concept("al_pll_cp",         "pll_cdr", "Charge pump mismatch — reference spurs, dead zone", 4),
            Concept("al_pll_layout",     "pll_cdr", "PLL layout — VCO isolation, supply filtering, guard rings", 2),
        ],
        "adc_dac": [
            Concept("al_adc_types",      "adc_dac", "ADC architectures — SAR, sigma-delta, flash, pipeline", 0),
            Concept("al_adc_sar",        "adc_dac", "SAR ADC — DAC capacitor array, comparator, switching scheme", 2),
            Concept("al_adc_sdm",        "adc_dac", "Sigma-delta — oversampling, noise shaping, decimation", 3),
            Concept("al_adc_dnl_inl",    "adc_dac", "DNL/INL — what causes them, layout impact", 2),
            Concept("al_dac_basics",     "adc_dac", "DAC architectures — R-2R, current-steering, capacitive", 1),
            Concept("al_dac_matching",   "adc_dac", "DAC matching — unit element, DEM, grading", 3),
            Concept("al_adc_layout",     "adc_dac", "Data converter layout — capacitor matching, shielding, substrate noise", 3),
        ],
        "power_management": [
            Concept("al_pm_buckboost",   "power_management", "Buck vs boost vs LDO — when to use which", 0),
            Concept("al_pm_switching",   "power_management", "Switching regulator — inductor sizing, ripple, efficiency", 2),
            Concept("al_pm_gate_driver", "power_management", "Gate driver design — dead time, shoot-through prevention", 2),
            Concept("al_pm_uvlo",        "power_management", "UVLO and OVP — protection circuits", 1),
            Concept("al_pm_thermal",     "power_management", "Thermal shutdown and current limiting", 3),
            Concept("al_pm_layout",      "power_management", "Power IC layout — high current routing, thermal vias, Kelvin connections", 3),
        ],
    },
